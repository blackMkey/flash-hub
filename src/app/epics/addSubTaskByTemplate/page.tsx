...
function AddSubTaskByTemplateContent() {
  ...
  const { data: fetchedEpic, error: epicQueryError } = useEpic(epicKey || null);
  ...
  const handleCreateSubtasks = async () => {
    ...
    const parentKeyToUse = isMultiEpicMode
      ? "MULTI-EPIC"
      : currentEpic?.key || "";
    const projectKeyToUse = isMultiEpicMode
      ? "MULTI-PROJECT"
      : currentEpic?.project.key || "";
  ...
  const updateSubtaskState = (
    index: number,
    newState: "idle" | "uploading" | "complete" | "error"
  ) => {
    ...
    <Box p={6} maxW="4xl" mx="auto">
      ...
      {isMultiEpicMode ? (
        ...
        >
          <Text fontSize="sm" color="gray.600" mb={2}>
            Multi-Epic Subtask Creation Mode
          </Text>
          <Flex align="center" gap={3}>
            ...
            <Text fontWeight="medium">
              Create subtasks across multiple epics
            </Text>
          </Flex>
        </Box>
      ) : (
        currentEpic && (
          ...
          >
            <Text fontSize="sm" color="gray.600" mb={2}>
              Creating subtasks by template for:
            </Text>
            <Flex align="center" gap={3}>
      ...
      {exportError && (
        <Box
          ...
          mb={6}
        >
          <Text color="red.800">❌ Template Error: {exportError}</Text>
        </Box>
      )}
        ...
        <Stack gap={6}>
          ...
          <Box>
            <Text fontSize="lg" fontWeight="bold" mb={2}>
              {isMultiEpicMode
                ? "🌟 Create Multi-Epic Subtasks by Template"
                : "📋 Create Subtasks by Template"}
            </Text>
            ...
            {isMultiEpicMode ? (
              ...
              >
                ...
                <Text fontSize="sm" color="purple.700">
                  Template includes an Epic column. You can specify different
                  epic keys for each subtask.
                </Text>
              </Box>
            ) : (
              <Box
              ...
              >
                ...
                <Text fontSize="sm" color="blue.700">
                  Creates all subtasks in a single API call for better
                ...
                <Stack gap={1} mt={1}>
                  {currentEpic?.assignee?.emailAddress && (
                    <Text fontSize="sm" color="green.700">
                      • Assignee:{" "}
                      {currentEpic.assignee.emailAddress.split(" ")[0]} (from
                      epic assignee)
                    </Text>
                  )}
                  {currentEpic?.reporter?.emailAddress && (
                    <Text fontSize="sm" color="green.700">
                      • Reporter:{" "}
                      {currentEpic.reporter.emailAddress.split(" ")[0]} (from
                      epic reporter)
                    </Text>
                  )}
                </Stack>
          ...
          <Flex gap={6} align="start">
            ...
            <Box flex="1">
              ...
              <Text fontSize="sm" color="gray.500" mt={2}>
                {uploadedFile
                  ? `Selected: ${(uploadedFile.size / 1024 / 1024).toFixed(
                      2
                    )} MB`
                ...
                <Box>
                  ...
                  {parseResult.errors.length > 0 && (
                    ...
                    >
                      <Text color="red.800" fontWeight="medium" mb={2}>
                        ⚠️ Found {parseResult.errors.length} error(s) in your
                        template:
                      </Text>
                      <Stack gap={1}>
                  ...
                  >
                    <Flex justify="space-between" align="center">
                      <Text color="blue.800">
                        📊 Found <strong>{parseResult.subtasks.length}</strong>{" "}
                        subtasks to create
                      </Text>
                      <Badge
                        colorPalette={
                          parseResult.errors.length > 0 ? "red" : "green"
                        }
                      >
                        {parseResult.errors.length > 0
                  ...
                  {parseResult.subtasks.length > 0 && (
                    <Box>
                      <Text fontSize="sm" color="gray.600" mb={3}>
                        All {parseResult.subtasks.length} subtasks from your
                        template:
                      </Text>
                      <Box
                        overflowX="auto"
                              ...
                              <Table.Row key={index}>
                                ...
                                <Table.Cell>
                                  <Badge
                                    size="sm"
                                    colorPalette={
                                      STATE_COLORS[subtask.state] || "gray"
                                    }
                                    cursor="pointer"
                  ...
                  <Flex justify="center" mt={6}>
                    ...
                    >
                      {isCreating ? (
                        <Flex align="center" gap={2}>
                          <Spinner size="sm" />
                          Creating {parseResult.subtasks.length} Subtasks in
                          Bulk...
                        </Flex>
                      ) : (
                      ...
                      <Stack gap={4}>
                            ...
                            <Stack gap={2}>
                              ...
                              {bulkResult.successful.length > 5 && (
                                <Text
                                  ...
                                  fontStyle="italic"
                                >
                                  ... and {bulkResult.successful.length - 5}{" "}
                                  more subtasks created
                                </Text>
                              )}
                            </Stack>
                          </Box>
                        ...
                        >
                          ...
                          <Stack gap={2}>
                            {bulkResult.totalSuccessful > 0 && (
                              <Text fontSize="sm" color="purple.700">
                                • View your new subtasks in the epic:{" "}
                                {currentEpic?.key}
                              </Text>
                            )}
                            {bulkResult.totalFailed > 0 && (
                              <Text fontSize="sm" color="purple.700">
                                • Fix errors in template and retry failed
                                subtasks
                              </Text>
                            )}
                            <Text fontSize="sm" color="purple.700">
                              • Return to epic page to see progress and manage
                              subtasks
                            </Text>
                          </Stack>
                        </Box>
