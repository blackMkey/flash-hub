...
export class ExcelService {
  ...
  static async exportEpicToExcel(
    ...
    const { includeSubtasks = true, sheetName = "Epic Details" } = options;

    // Create a new workbook
    const workbook = new ExcelJS.Workbook();

    // Set workbook properties
    workbook.creator = "theFlash";
    workbook.lastModifiedBy = "theFlash";
    ...
    subtasks.forEach((subtask, index) => {
      ...
      row.values = {
        ...
        created: subtask.created
          ? new Date(subtask.created).toLocaleDateString()
          : "N/A",
        updated: subtask.updated
          ? new Date(subtask.updated).toLocaleDateString()
          : "N/A",
      };
  ...
  static async createSubtaskTemplate(
    ...
    const epicAssigneeUsername = epicAssigneeEmail
      ? extractUsernameFromEmail(epicAssigneeEmail)
      : "";
    const epicReporterUsername = epicReporterEmail
      ? extractUsernameFromEmail(epicReporterEmail)
      : "";

    const defaultAssignee =
      epicAssigneeUsername || currentUserName || "john.doe";
    const defaultReporter =
      epicReporterUsername || currentUserName || "john.doe";
    ...
    worksheet.addRow([
      "- Fill in Summary, Description, and Type of Work for each subtask",
    ]);
    worksheet.addRow(["- Use Component IDs from the Components sheet"]);
    worksheet.addRow(["- Use Product IDs from the Products sheet"]);
    worksheet.addRow([
      "- Use Type of Work values from the Types of Work sheet",
    ]);
    if (isMultiEpic) {
      worksheet.addRow([
        "- Epic Key: Specify the epic key for each subtask (e.g., EPIC-123)",
      ]);
    ...
    worksheet.addRow([
      "- Assignee defaults to epic assignee, then your username, but can be changed",
    ]);
    worksheet.addRow([
      "- Reporter defaults to epic reporter, then your username, but can be changed",
    ]);
  ...
  static async parseSubtaskTemplate(file: File): Promise<ParseResult> {
    ...
    try {
      ...
      worksheet.eachRow({ includeEmpty: false }, (row, rowNumber) => {
        ...
        const priority =
          totalColumns >= 13
            ? this.getCellValue(row, 13)
            : totalColumns >= 12
            ? null
            : this.getCellValue(row, 12); // Adjust priority column
