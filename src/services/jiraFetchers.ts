...
const calculateProgress = (subtasks: JiraSubtask[]): { percent: number; total: number; progress: number } => {
  if (!subtasks.length) {
...
export const fetchEpicByKey = async (epicKey: string): Promise<JiraEpic> => {
  const response = await fetch(`/api/jira?endpoint=${encodeURIComponent(`issue/${epicKey}?expand=names,schema`)}`, {
    method: 'GET',
    headers: {
      'Authorization': `Bearer ${TokenStorage.getEncodedToken()}`,
  ...
  if (!response.ok) {
    ...
    throw new Error(errorData.error || `Failed to fetch epic: ${response.status}`)
  }

  const issue = await response.json()
  
  if (!issue || issue.fields?.issuetype?.name !== 'Epic') {
    throw new Error(`Issue ${epicKey} is not an Epic. It's a ${issue.fields?.issuetype?.name}`)
  }
  ...
  return {
    ...
    subtasks: fields.subtasks?.map((subtask: JiraSubtask) => ({
      id: subtask.id,
      key: subtask.key,
...
export const fetchComponents = async (projectId: string): Promise<Component[]> => {
...
export const fetchTypeOfWork = async (projectId: string, issueKey: string): Promise<TypeOfWorkOption[]> => {
  const response = await fetch(`/api/jira/type-of-work?projectId=${projectId}&issueKey=${issueKey}`, {
    headers: {
      'Authorization': `Bearer ${TokenStorage.getEncodedToken()}`
    }
  })
  ...
  if (Array.isArray(data.typeOfWork?.cfTypeOfWorkData)) {
    return data.typeOfWork.cfTypeOfWorkData.map((option: { id: string; text: string }) => ({
      value: option.id,
      label: option.text
    }))
  }
...
export const testConnection = async (): Promise<{ success: boolean; user?: User; error?: string }> => {
  try {
    const response = await fetch(`/api/jira?endpoint=${encodeURIComponent('myself')}`, {
      method: 'GET',
      headers: {
        'Authorization': `Bearer ${TokenStorage.getEncodedToken()}`,
        'Accept': 'application/json',
...
export const fetchDefectPatterns = async (issueId: string, projectId: string): Promise<DefectPattern[]> => {
  const response = await fetch(`/api/jira/defect-patterns?issueId=${issueId}&projectId=${projectId}`, {
    method: 'GET',
    headers: {
      'Content-Type': 'application/json',
      'Authorization': `Bearer ${TokenStorage.getEncodedToken()}`
  ...
  for (let i = 0; i < subtasksData.length; i++) {
    ...
    try {
      ...
      const payload: CreateSubtaskPayload = {
        ...
        remainingEstimate: subtaskData.remainingEstimate || subtaskData.originalEstimate || ''
      }
...
export const fetchEpicParentKey = async (epicKey: string): Promise<string> => {
  ...
  if (!response.ok) {
    const errorData = await response.json()
    throw new Error(errorData.error || `Failed to fetch epic parent key: ${response.status}`)
  }

  const result = await response.json()
  
  if (result.errors && result.errors.length > 0) {
    throw new Error(`Failed to fetch parent key for epic ${epicKey}: ${result.errors[0].error}`)
  }
 ...
 */
export const fetchEpicParentKeys = async (epicKeys: string[]): Promise<Record<string, string>> => {
  const response = await fetch('/api/jira/epic-parent-keys', {
    method: 'POST',
    headers: {
      'Content-Type': 'application/json',
  ...
  if (!response.ok) {
    const errorData = await response.json()
    throw new Error(errorData.error || `Failed to fetch epic parent keys: ${response.status}`)
  }
  ...
  if (result.errors && result.errors.length > 0) {
    const failedEpics = result.errors.map((err: { epicKey: string; error: string }) => err.epicKey)
    throw new Error(`Failed to fetch parent keys for epics: ${failedEpics.join(', ')}`)
  }
